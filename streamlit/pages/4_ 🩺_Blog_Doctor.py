...
# Helper function to calculate the mmrlift score
def mmr_lift_score(text):
    ...
    def get_lift(keyword):
        comp_time = (date.today() -timedelta(days=365)).isoformat() + " " + date.today().isoformat()
        ...
        month_score = loc_df.iloc[-1][keyword]
        mean = loc_df[keyword].mean()

        return month_score/(max(mean, 1))
    
    lift_sum = 0.0

    for key_word in get_mmr_keywords(text):
        lift_sum += get_lift(key_word)
...
def analyze(blogtext):
    ...
    out['lift score'] = (
       round(mmr_lift_score(blogtext), 2), 
       'mmr_lift',
       '''
        Mmr Lift Score measures how closely the most used keywords of your text align with recent Google search trends. The score measures above 5 if the keywords were more popular this month than on average within the year, and below 5 otherwise.
       '''
    )
